...
const activityListing = async (filterObject, limit, offset) => {
  let query = `select * from activity_service.activity_listing($1,$2,$3,$4);`;
  const values = [
    filterObject.city,
...
const searchActivitiesByDate = async (filterObject, limit, offset) => {
  let query = `select * from activity_service.activity_search($1,$2,$3,$4,$5,$6);`;
  let values = [
    filterObject.text,
    filterObject.city,
    filterObject.date,
...
const searchActivitiesByDateRange = async (filterObject, limit, offset) => {
  ...
  let query = `select * from activity_service.activity_search_by_date($1,$2,$3,$4,$5,$6,$7);`;
  let values = [
    filterObject.text,
    filterObject.city,
